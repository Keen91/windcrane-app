...
export const signin = async ({ email, password }) => {
  const response = await axios.post('http://user.windcrane.com/manager/api/v1/login?email=windcrane.test', JSON.stringify({ email, password }), {
...
export const signout = async () => {
  const response = await axios.get(`http://user.windcrane.com/manager/api/v1/logout`, {
...
export const getDeviceLists = async (pageParam = 1) => {
  ...
  const response = await axios.get(`http://user.windcrane.com/manager/api/v1/device/list?page=${pageParam}`, {
    headers: { Authorization: `Bearer ${localStorage.getItem('accessToken')}` },
    withCredentials: true
  });
  // console.log(response.data);
...
export const getHistoricChartData = async (device1, ...others) => {
  ...
  // One device selected
  if (others.length === 0) {
    link = `http://user.windcrane.com/manager/api/v1/historic?s=${device1},A1,lastmax&r=week&i=minute&n=1`;
  }
  // Multiple devices selected
  else if (others.length > 0) {
    const multipleDevices = others.map(device => `;${device},A1,generated`);
    link = `http://user.windcrane.com/manager/api/v1/historic?s=${device1},A1,lastmax${multipleDevices.map(d => d)}&r=week&i=minute&n=1`;
